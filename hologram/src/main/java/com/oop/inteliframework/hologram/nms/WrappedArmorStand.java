package com.oop.inteliframework.hologram.nms;
...
public interface WrappedArmorStand extends VersionedAdapter {
    BiFunction<Location, HologramLine, WrappedArmorStand> supplier = requestFunction();

    static BiFunction<Location, HologramLine, WrappedArmorStand> requestFunction() {
        Optional<Class<WrappedArmorStand>> wrappedArmorStandClass = VersionedAdapters.loadClass(WrappedArmorStand.class.getPackage().getName(), "ImplWrappedArmorStand", WrappedArmorStand.class);
        if (!wrappedArmorStandClass.isPresent())
            throw new IllegalStateException(format("Unsupported Version ({}), no class found.", VersionedAdapters.version));

        Constructor<WrappedArmorStand> constructor;
        try {
            Class<WrappedArmorStand> clazz = wrappedArmorStandClass.get();
            constructor = clazz.getDeclaredConstructor(Location.class, HologramLine.class);
            constructor.setAccessible(true);
        } catch (Throwable throwable) {
            throw new IllegalStateException(format("Unsupported Version ({}), no constructor found.", VersionedAdapters.version));
        }
