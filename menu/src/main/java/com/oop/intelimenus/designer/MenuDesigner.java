...
public class MenuDesigner<T extends InteliMenu, P> {
    ...
    private static Map<Character, IButton> mapOfArray(Object... objects) {
        ...
        while (inside) {
            ...
            Preconditions.checkArgument(key instanceof Character,
                "Key is not a character! (" + key + ") at" + i);
            Preconditions.checkArgument(value instanceof IButton,
                "Value is not a button! (" + value + ") at " + i);
    ...
    public MenuDesigner<T, P> rows(@NonNull String design,
        @NonNull Map<Character, IButton> charDesign, int... rows) {
        for (int row : rows) {
    ...
    public MenuDesigner<T, P> rows(int from, int to, @NonNull String design,
        @NonNull Map<Character, IButton> charDesign) {
        IntStream.range(from, to).forEach(row -> row(row, design, charDesign));
        return this;
    }

    public MenuDesigner<T, P> rows(int[] rows, @NonNull String design,
        @NonNull Object... charDesign) {
    ...
    public MenuDesigner<T, P> row(int row, @NonNull String design,
        @NonNull Map<Character, IButton> charDesign) {
        design = design.replaceAll("\\s+", "");
        Preconditions.checkArgument(design.length() == 9,
            "Incorrect design passed. The length should be 9! (" + design.length() + "/9) = "
                + design);
    ...
    public MenuDesigner<T, P> fillBorder(@NonNull FillBorder border, @NonNull IButton button) {
        ...
        Function<Integer, int[]> slotsByRow = row -> {
