...
public abstract class MenuBuilder<T extends InteliMenu, B extends MenuBuilder<T, B>> {
    ...
    public static <T extends InteliPagedMenu<O>, O> PlayerPasser<PagedMenuBuilder<T, O>> customPagedMenu(
    ...
    public B size(int size) {
        Preconditions.checkArgument(size < 55 && size % 9 == 0,
    ...
    public B rebuildOnOpen() {
        apply(menu -> menu.applyComponent(AttributeComponent.class,
            comp -> comp.addAttribute(Attributes.REBUILD_ON_OPEN)));
        return (B) this;
    }
    ...
    public static class PagedMenuBuilder<T extends InteliPagedMenu<O>, O> extends
        MenuBuilder<T, PagedMenuBuilder<T, O>> {
    ...
    public static class SimpleMenuBuilder<T extends InteliMenu> extends
        MenuBuilder<T, SimpleMenuBuilder<T>> {
