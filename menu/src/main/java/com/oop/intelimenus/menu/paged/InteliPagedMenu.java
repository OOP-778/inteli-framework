...
public class InteliPagedMenu<T extends Object> extends InteliMenu {
        ...
        applyComponent(StateRequestComponent.class, src -> {
            src.register(slot ->
                    slot.getHolder().isPresent()
                        ...
                        && slot.getHolder().get().getComponent(AttributeComponent.class)
                        .filter(ac -> ac.hasAttribute(Attributes.NEXT_PAGE.get())).isPresent(),
                slot -> {
                    ...
                    return slot.getHolder().get().getComponent(StateComponent.class).get()
                        ...
                        .orElseThrow(() -> new IllegalStateException(
                            "Failed to find state for next page button for by " + (pagePresent
                                ? "page-available" : "page-not-available")));
                });
            src.register(slot ->
                    slot.getHolder().isPresent()
                        && slot.getHolder().get().getComponent(StateComponent.class).isPresent()
                        && slot.getHolder().get().getComponent(AttributeComponent.class)
                        .filter(ac -> ac.hasAttribute(Attributes.PREVIOUS_PAGE.get())).isPresent(),
                slot -> {
                    boolean pagePresent = currentPage != 1;
                    return slot.getHolder().get().getComponent(StateComponent.class).get()
                        .getState(pagePresent ? "page-available" : "page-not-available")
                        .orElseThrow(() -> new IllegalStateException(
                            "Failed to find state for previous page button for by " + (pagePresent
                                ? "page-available" : "page-not-available")));
                });
        });
    }
    ...
    public Inventory buildMenu() {
        Preconditions.checkArgument(objectsProvider != null,
            "Failed to build cause objectsProvider is not set!");
        Preconditions.checkArgument(pagedButtonBuilder != null,
            "Failed to build cause pagedButtonBuilder is not set!");
        ...
        int pages = freeSlots.length >= ts.size() ? 1
            : (int) Math.ceil((float) ts.size() / freeSlots.length);
        ...
        for (InteliSlot freeSlot : freeSlots) {
            ...
            objectButton
                .applyComponent(DataComponent.class, c -> c.add("objectIndex", finalObjectIndex));

            freeSlot.setHolder(objectButton);
            inventory.setItem(freeSlot.getIndex(), objectButton.getCurrentItem().map(Supplier::get)
                .orElse(new ItemStack(Material.BARRIER)));

            objectIndex++;
        }

        Optional<InteliSlot> optNextPageSlot = getInventoryData()
            .findSlot(slot -> slot.getHolder().isPresent()
                && slot.getHolder().get().getComponent(StateComponent.class).isPresent()
                && slot.getHolder().get().getComponent(AttributeComponent.class)
                .filter(ac -> ac.hasAttribute(Attributes.NEXT_PAGE.get())).isPresent());

        Optional<InteliSlot> optPreviousPageSlot = getInventoryData()
            .findSlot(slot -> slot.getHolder().isPresent()
                && slot.getHolder().get().getComponent(StateComponent.class).isPresent()
                && slot.getHolder().get().getComponent(AttributeComponent.class)
                .filter(ac -> ac.hasAttribute(Attributes.PREVIOUS_PAGE.get())).isPresent());
        ...
        if (title.contains("{pages}") || title.contains("{currentPage}")) {
            getInventoryData().updateTitle(
                title.replace("{pages}", getInventoryData().getPages() + "")
    ...
    public PagedInventoryData<T> getInventoryData() {
        ...
        if (!(inventoryData instanceof PagedInventoryData)) {
            setInventoryData(new PagedInventoryData<>(this, inventoryData.getSlots().length,
