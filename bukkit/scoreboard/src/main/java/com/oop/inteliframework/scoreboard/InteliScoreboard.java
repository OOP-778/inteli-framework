...
public class InteliScoreboard {
  public static final String[] lineIdentifiers =
  ...
  private final ConcurrentObject<InsertableList<LineEntry>> linesObject =
      new ConcurrentObject<>(new InsertableList<>());
  // Lines cache
  ...
  @Setter @NonNull private Function<Player, String> titleSupplier;
  ...
  public static String[] splitIntoParts(String input) {
    if (InteliVersion.isOrAfter(13)) return new String[] {input};
    ...
    for (int i = 0; i < input.length(); i++) {
      ...
      if ((input.charAt(i) == '&' || input.charAt(i) == ChatColor.COLOR_CHAR)
          && colors.contains((input.charAt(i + 1)))) {
      ...
      if ((input.charAt(i) == '&' || input.charAt(i) == ChatColor.COLOR_CHAR)
          && decorators.contains((input.charAt(i + 1)))) {
  ...
  public void remove() {
    for (Player viewer : viewers) remove(viewer);
  }
  ...
  public void remove(Player player) {
    ...
    linesObject.modify(
        lines -> {
          ScoreboardCache playerCache =
              userCache.computeIfAbsent(player.getUniqueId(), k -> new ScoreboardCache());
          ...
          lines -> {
            for (int i = 0; i < lines.size(); i++) {
              ...
              if (oldLine == null || !line.equalsIgnoreCase(oldLine)) {
                ...
                if (oldLine != null)
                  adapter.sendTeam(
                      this, lineIdentifiers[i], parts, SbAdapter.TeamAction.UPDATE, player);
                else
                  adapter.sendTeam(
                      this, lineIdentifiers[i], parts, SbAdapter.TeamAction.CREATE, player);
  ...
  public void updateAll() {
    for (Player viewer : viewers) update(viewer);
  }
}
