...
public abstract class SbAdapter {
  ...
  protected static Constructor<?> SB_OBJECTIVE_PACKET_CONSTRUCTOR,
      SB_OBJECTIVE_PACKET_DISPLAY_CONSTRUCTOR,
      SB_SCORE_PACKET_CONSTRUCTOR,
      SB_TEAM_PACKET_CONSTRUCTOR;
  protected static Enum<?> ENUM_SB_HEALTH_INTEGER,
  ...
  static {
    if (InteliVersion.is(8)) implementation = new SbAdapter_1_8();
    else if (InteliVersion.is(12)) implementation = new SbAdapter_1_12();
    else if (InteliVersion.is(13)) implementation = new SbAdapter_1_13();
    else if (InteliVersion.is(14)) implementation = new SbAdapter_1_14();
    else if (InteliVersion.is(15)) implementation = new SbAdapter_1_15();
    else if (InteliVersion.is(16)) implementation = new SbAdapter_1_16();
    else
      throw new Error(
          "Unsupported version " + InteliVersion.getStringVersion() + " for scoreboard!");
  }
  ...
  public SbAdapter() {
    try {
      ...
      Method CONNECTION_SEND_PACKET =
          SimpleReflection.getMethod(
              PLAYER_CONNECTION_CLASS, "sendPacket", SimpleReflection.findClass("{nms}.Packet"));
      ...
      if (InteliVersion.isOrAfter(8) && InteliVersion.isBefore(13)) {
        ENUM_SB_ACTION_CLASS =
            SimpleReflection.findClass("{nms}.PacketPlayOutScoreboardScore$EnumScoreboardAction");
      ...
      if (InteliVersion.isOrAfter(13)) {
        ...
        ENUM_SB_ACTION_CHANGE_1_13 =
            Enum.valueOf((Class<Enum>) ENUM_SB_ACTION_CLASS_1_13, "CHANGE");
        ENUM_SB_ACTION_REMOVE_1_13 =
            Enum.valueOf((Class<Enum>) ENUM_SB_ACTION_CLASS_1_13, "REMOVE");
      }

      ENUM_SB_DISPLAY_CLASS =
          SimpleReflection.findClass("{nms}.IScoreboardCriteria$EnumScoreboardHealthDisplay");
      ENUM_SB_HEALTH_INTEGER = Enum.valueOf((Class<Enum>) ENUM_SB_DISPLAY_CLASS, "INTEGER");

      MESSAGE_FROM_STRING =
          SimpleReflection.getMethod(CRAFT_CHAT_MESSAGE_CLASS, "fromString", String.class);
          ...
          (player, packet) -> {
            try {
              CONNECTION_SEND_PACKET.invoke(
                  PLAYER_CONNECTION.get(PLAYER_GET_HANDLE.invoke(player)), packet);
              // printOutPacket(packet);
            } catch (Throwable throwable) {
              throw new IllegalStateException(
                  "Failed to send packet to " + player.getName(), throwable);
            }
          };
      ...
      throw new IllegalStateException(
          "Failed to construct adapter of " + getClass().getSimpleName(), throwable);
    }
  }
  ...
  protected void setCompField(Object where, String fieldName, String value) {
    try {
      ...
      } else field.set(where, value);
    } catch (Throwable throwable) {
      throw new IllegalStateException(
          "Failed to set component field " + where + " value: " + value, throwable);
    }
  }
  ...
  protected void setField(Object where, String fieldName, Object to) {
    try {
      ...
      throw new IllegalStateException(
          "Failed to set field of name `"
              + fieldName
              + "` of "
              + where.getClass().getSimpleName()
              + " to "
              + to,
          throwable);
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          "Failed to send objective "
              + objectiveAction.name()
              + " to "
              + Arrays.stream(players).map(HumanEntity::getName).collect(Collectors.joining(", ")),
          throwable);
    }
  }
  ...
  public void sendDisplayObjective(InteliScoreboard scoreboard, Player... players) {
    try {
      ...
      throw new IllegalStateException(
          "Failed to send display objective to "
              + Arrays.stream(players).map(HumanEntity::getName).collect(Collectors.joining(", ")),
          throwable);
    }
  }
  ...
  protected abstract void _sendObjective(
      InteliScoreboard scoreboard, ObjectiveAction objectiveAction, Player... players)
      throws Throwable;

  protected abstract void _sendDisplayObjective(InteliScoreboard scoreboard, Player... players)
      throws Throwable;

  protected abstract void _sendScore(
      InteliScoreboard scoreboard, int line, ScoreAction scoreAction, Player... players)
      throws Throwable;

  protected abstract void _sendTeam(
      InteliScoreboard scoreboard,
      String identifier,
      String[] parts,
      TeamAction teamAction,
