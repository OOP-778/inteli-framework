package com.oop.inteliframework.entity.armorstand.nms.V1_12_R1;
...
import static com.oop.inteliframework.entity.armorstand.nms.V1_12_R1.Helper.constructEntityMetaPacket;
...
public class WrappedItem {
  ...
  public Packet constructSpawnPacket() {
    return new PacketPlayOutSpawnEntity(entityItem, 2, 1);
  }

  public DataWatcherHelper getDataWatcher(Player player) {
    return watcherMap.computeIfAbsent(
        player.getUniqueId(), key -> new DataWatcherHelper(entityItem));
  }

  public Packet constructUpdatePacket(Player player) {
    return constructEntityMetaPacket(
        getDataWatcher(player).getWatchableObjects(), entityItem.getId());
  }
  ...
  public net.minecraft.server.v1_12_R1.ItemStack getItem(Player player) {
  ...
  public void setItem(ItemStack item, Player player) {
    getDataWatcher(player).setItemStack(CraftItemStack.asNMSCopy(item));
  }
}
