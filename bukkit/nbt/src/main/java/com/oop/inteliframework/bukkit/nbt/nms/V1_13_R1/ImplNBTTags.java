package com.oop.inteliframework.bukkit.nbt.nms.v1_13_R1;
...
public class ImplNBTTags implements NBTTags {
  ...
  public CompoundTag fromItemStack(ItemStack itemStack) {
    final net.minecraft.server.v1_13_R1.ItemStack nmsCopy = CraftItemStack.asNMSCopy(itemStack);
  ...
  public ListTag convertListTag(NBTTagList tag) {
    ListTag converted = ListTag.createUnchecked(TagIdToClass.getClassFromId(tag.d()));
    if (tag.isEmpty()) {
      return converted;
    }
  ...
  public Tag<?> convertSimpleTag(NBTBase tag) {
    if (tag instanceof NBTTagString) {
      return new StringTag(tag.b_());
    }

    if (tag instanceof NBTTagInt) {
      return new IntTag(((NBTTagInt) tag).e());
    }

    if (tag instanceof NBTTagLong) {
      return new LongTag(((NBTTagLong) tag).d());
    }

    if (tag instanceof NBTTagShort) {
      return new ShortTag(((NBTTagShort) tag).f());
    }
    ...
    if (tag instanceof NBTTagFloat) {
      return new FloatTag(((NBTTagFloat) tag).i());
    }
    ...
    if (tag instanceof NBTTagIntArray) {
      return new IntArrayTag(((NBTTagIntArray) tag).d());
    }

    if (tag instanceof NBTTagByte) {
      return new ByteTag(((NBTTagByte) tag).g());
    }
  ...
  public ItemStack setItemStackTag(ItemStack itemStack, CompoundTag compoundTag) {
    final net.minecraft.server.v1_13_R1.ItemStack nmsItem = CraftItemStack.asNMSCopy(itemStack);
