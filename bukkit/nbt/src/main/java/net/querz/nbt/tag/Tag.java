 ...
 * circular references or malicious data which could, when deserialized, result in thousands of
 * instances causing a denial of service.
 *
 * <p>These methods have a parameter for the maximum nesting depth they are allowed to traverse. A
 * value of {@code 0} means that only the object itself, but no nested objects may be processed. If
 * an instance is nested further than allowed, a {@link MaxDepthReachedException} will be thrown.
 * Providing a negative maximum nesting depth will cause an {@code IllegalArgumentException} to be
 * thrown.
 *
 ...
 *
 * <p>If custom NBT tags contain objects other than NBT tags, which can be nested as well, then
 * there is no guarantee that {@code MaxDepthReachedException}s are thrown for them. The respective
 * class will document this behavior accordingly.
 *
...
public abstract class Tag<T> implements Cloneable {
  ...
  /**
   * Initializes this Tag with some value. If the value is {@code null}, it will throw a {@code
  ...
  /**
   * Escapes a string to fit into a JSON-like string representation for Minecraft or to create the
   * JSON string representation of a Tag returned from {@link Tag#toString()}
   *
   * @param s The string to be escaped.
   * @param lenient {@code true} if it should force double quotes ({@code "}) at the start and the
   *     end of the string.
  ...
  /**
   * Returns whether this Tag and some other Tag are equal. They are equal if {@code other} is not
   * {@code null} and they are of the same class. Custom Tag implementations should overwrite this
   * but check the result of this {@code super}-method while comparing.
   *
  ...
  /**
   * Calculates the hash code of this Tag. Tags which are equal according to {@link
   * Tag#equals(Object)} must return an equal hash code.
   *
