...
/**
 * ListTag represents a typed List in the nbt structure. An empty {@link ListTag} will be of type
 * {@link EndTag} (unknown type). The type of an empty untyped {@link ListTag} can be set by using
 * any of the {@code add()} methods or any of the {@code as...List()} methods.
 */
public class ListTag<T extends Tag<?>> extends Tag<List<T>>
    implements Iterable<T>, Comparable<ListTag<T>>, MaxDepthIO {
  ...
  /**
   * Creates a non-type-safe ListTag. Its element type will be set after the first element was
   * added.
   *
   * <p>This is an internal helper method for cases where the element type is not known at
   * construction time. Use {@link #ListTag(Class)} when the type is known.
   *
  ...
  public String valueToString(int maxDepth) {
    StringBuilder sb =
        new StringBuilder("{\"type\":\"")
            .append(getTypeClass().getSimpleName())
            .append("\",\"list\":[");
  ...
  public boolean equals(Object other) {
    ...
    if (!super.equals(other)
        || size() != ((ListTag<?>) other).size()
        || getTypeClass() != ((ListTag<?>) other).getTypeClass()) {
      return false;
    }
    for (int i = 0; i < size(); i++) {
