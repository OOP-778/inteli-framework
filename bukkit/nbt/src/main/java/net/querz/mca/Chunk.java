...
public class Chunk {
  ...
  private void initReferences(long loadFlags) {
    ...
    if ((loadFlags & ENTITIES) != 0) {
      entities =
          level.containsKey("Entities") ? level.getListTag("Entities").asCompoundTagList() : null;
    }
    if ((loadFlags & TILE_ENTITIES) != 0) {
      tileEntities =
          level.containsKey("TileEntities")
              ? level.getListTag("TileEntities").asCompoundTagList()
              : null;
    }
    if ((loadFlags & TILE_TICKS) != 0) {
      tileTicks =
          level.containsKey("TileTicks") ? level.getListTag("TileTicks").asCompoundTagList() : null;
    }
    if ((loadFlags & LIQUID_TICKS) != 0) {
      liquidTicks =
          level.containsKey("LiquidTicks")
              ? level.getListTag("LiquidTicks").asCompoundTagList()
              : null;
    }
    ...
    if ((loadFlags & LIQUIDS_TO_BE_TICKED) != 0) {
      liquidsToBeTicked =
          level.containsKey("LiquidsToBeTicked")
              ? level.getListTag("LiquidsToBeTicked").asListTagList()
              : null;
    }
    if ((loadFlags & TO_BE_TICKED) != 0) {
      toBeTicked =
          level.containsKey("ToBeTicked") ? level.getListTag("ToBeTicked").asListTagList() : null;
    }
    if ((loadFlags & POST_PROCESSING) != 0) {
      postProcessing =
          level.containsKey("PostProcessing")
              ? level.getListTag("PostProcessing").asListTagList()
              : null;
    }
    ...
    // If we haven't requested the full set of data we can drop the underlying raw data to let the
    // GC handle it.
  ...
  public int serialize(RandomAccessFile raf, int xPos, int zPos) throws IOException {
    ...
    try (BufferedOutputStream nbtOut =
        new BufferedOutputStream(CompressionType.ZLIB.compress(baos))) {
  ...
  /**
   * Reads chunk data from a RandomAccessFile. The RandomAccessFile must already be at the correct
   * position.
   *
   ...
   *
   * @param raf The RandomAccessFile to read the chunk data from.
   * @param loadFlags A logical or of {@link LoadFlags} constants indicating what data should be
   *     loaded
   * @throws IOException When something went wrong during reading.
   */
  public void deserialize(RandomAccessFile raf, long loadFlags) throws IOException {
    ...
    BufferedInputStream dis =
        new BufferedInputStream(compressionType.decompress(new FileInputStream(raf.getFD())));
  ...
  public int getBiomeAt(int blockX, int blockZ) {
    if (dataVersion < 2202) {
      ...
      throw new IllegalStateException(
          "cannot get biome using Chunk#getBiomeAt(int,int) from biome data with DataVersion of 2202 or higher, use Chunk#getBiomeAt(int,int,int) instead");
    }
  }

  /**
   * Fetches a biome id at a specific block in this chunk. The coordinates can be absolute
   * coordinates or relative to the region or chunk.
   *
  ...
  /**
   * Sets a biome id at a specific block column. The coordinates can be absolute coordinates or
   * relative to the region or chunk.
   *
   * @param blockX The x-coordinate of the block column.
   * @param blockZ The z-coordinate of the block column.
   * @param biomeID The biome id to be set. When set to a negative number, Minecraft will replace it
   *     with the block column's default biome.
   */
  ...
  /**
   * Sets a block state at a specific location. The block coordinates can be absolute or relative to
   * the region or chunk.
   *
   * @param blockX The x-coordinate of the block.
   * @param blockY The y-coordinate of the block.
   ...
   * @param cleanup When <code>true</code>, it will cleanup all palettes of this chunk. This option
   *     should only be used moderately to avoid unnecessary recalculation of the palette indices.
   *     Recalculating the Palette should only be executed once right before saving the Chunk to
   *     file.
   */
  public void setBlockStateAt(
      int blockX, int blockY, int blockZ, CompoundTag state, boolean cleanup) {
    checkRaw();
  ...
  /**
   * Sets the DataVersion of this chunk. This does not check if the data of this chunk conforms to
   * that DataVersion, that is the responsibility of the developer.
   *
   ...
   * @throws IllegalArgumentException When the biome matrix does not have a length of <code>256
   *     </code> or is <code>null</code>
   */
  public void setBiomes(int[] biomes) {
    checkRaw();
    if (biomes != null) {
      if (dataVersion < 2202 && biomes.length != 256
          || dataVersion >= 2202 && biomes.length != 1024) {
        throw new IllegalArgumentException(
            "biomes array must have a length of " + (dataVersion < 2202 ? "256" : "1024"));
      }
    }
    this.biomes = biomes;
  }
   ...
   *
   * @return The full chunk data or null if there is none, e.g. when this chunk has only been loaded
   *     partially.
   */
