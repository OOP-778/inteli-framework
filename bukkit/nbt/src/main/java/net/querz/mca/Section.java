...
public class Section {
  ...
  /**
   * Fetches a block state based on a block location from this section. The coordinates represent
   * the location of the block inside of this Section.
   *
   ...
   * @param cleanup When <code>true</code>, it will cleanup the palette of this section. This option
   *     should only be used moderately to avoid unnecessary recalculation of the palette indices.
   *     Recalculating the Palette should only be executed once right before saving the Section to
   *     file.
   */
  ...
  /**
   * Sets the index of the block data in the BlockStates. Does not adjust the size of the
   * BlockStates array.
   *
  ...
  public void setPaletteIndex(int blockIndex, int paletteIndex, long[] blockStates) {
    ...
    if (dataVersion < 2527) {
      ...
      if (startBit + bits > 64) {
        ...
        blockStates[longIndex + 1] =
            updateBits(
                blockStates[longIndex + 1], paletteIndex, startBit - 64, startBit + bits - 64);
      } else {
        blockStates[longIndex] =
            updateBits(blockStates[longIndex], paletteIndex, startBit, startBit + bits);
      }
    } else {
      int indicesPerLong = (int) (64D / bits);
      int blockStatesIndex = blockIndex / indicesPerLong;
      int startBit = (blockIndex % indicesPerLong) * bits;
      blockStates[blockStatesIndex] =
  ...
  /**
   * This method recalculates the palette and its indices. This should only be used moderately to
   * avoid unnecessary recalculation of the palette indices. Recalculating the Palette should only
   * be executed once right before saving the Section to file.
   */
   ...
   * @throws IllegalArgumentException When <code>blockStates</code>' length is &lt; 256 or &gt; 4096
  ...
  public void setBlockStates(long[] blockStates) {
    ...
    } else if (blockStates.length % 64 != 0
        || blockStates.length < 256
        || blockStates.length > 4096) {
      throw new IllegalArgumentException(
          "BlockStates must have a length > 255 and < 4097 and must be divisible by 64");
    }
  ...
  /**
   * Updates the raw CompoundTag that this Section is based on. This must be called before saving a
   * Section to disk if the Section was manually created to set the Y of this Section.
   *
