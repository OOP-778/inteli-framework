...
/**
 * Provides main and utility functions to read and write .mca files and to convert block, chunk and
 * region coordinates.
 */
public final class MCAUtil {

  private static final Pattern mcaFilePattern =
      Pattern.compile("^.*r\\.(?<regionX>-?\\d+)\\.(?<regionZ>-?\\d+)\\.mca$");
  ...
  /**
   * Writes an {@code MCAFile} object to disk. It optionally adjusts the timestamps when the file
   * was last saved to the current date and time or leaves them at the value set by either loading
   * an already existing MCA file or setting them manually.<br>
  ...
  /**
   * Turns the chunks coordinates into region coordinates and calls {@link
   * MCAUtil#createNameFromRegionLocation(int, int)}
   *
  ...
  /**
   * Turns the block coordinates into region coordinates and calls {@link
   * MCAUtil#createNameFromRegionLocation(int, int)}
   *
  ...
  public static MCAFile newMCAFile(File file) {
    ...
    if (m.find()) {
      return new MCAFile(
          Integer.parseInt(m.group("regionX")), Integer.parseInt(m.group("regionZ")));
    }
