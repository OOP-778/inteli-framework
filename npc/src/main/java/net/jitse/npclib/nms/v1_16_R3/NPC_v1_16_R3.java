...
public class NPC_v1_16_R3 extends NPCBase {
  ...
  public NPC_v1_16_R3(InteliNPCModule instance, List<String> lines) {
  ...
  public Hologram getHologram(Player player) {
    ...
    if (hologram == null)
      hologram =
          new Hologram(MinecraftVersion.V1_16_R3, location.clone().add(0, 0.5, 0), getText(player));
  ...
  public void createPackets(Player player) {
    ...
    PacketPlayOutPlayerInfoWrapper packetPlayOutPlayerInfoWrapper =
        new PacketPlayOutPlayerInfoWrapper();

    // Packets for spawning the NPC:
    this.packetPlayOutScoreboardTeamRegister =
        new PacketPlayOutScoreboardTeamWrapper().createRegisterTeam(name); // First packet to send.

    this.packetPlayOutPlayerInfoAdd =
        packetPlayOutPlayerInfoWrapper.create(
            PacketPlayOutPlayerInfo.EnumPlayerInfoAction.ADD_PLAYER,
            gameProfile,
            name); // Second packet to send.

    this.packetPlayOutNamedEntitySpawn =
        new PacketPlayOutNamedEntitySpawnWrapper()
            .create(uuid, location, entityId); // Third packet to send.

    this.packetPlayOutEntityHeadRotation =
        new PacketPlayOutEntityHeadRotationWrapper()
            .create(location, entityId); // Fourth packet to send.

    this.packetPlayOutPlayerInfoRemove =
        packetPlayOutPlayerInfoWrapper.create(
    ...
    this.packetPlayOutEntityDestroy =
        new PacketPlayOutEntityDestroy(entityId); // First packet to send.
  }
  ...
  public void sendMetadataPacket(Player player) {
    ...
    PacketPlayOutEntityMetadata packet =
        new PacketPlayOutEntityMetadataWrapper().create(activeStates, entityId);
  ...
  public void sendEquipmentPacket(Player player, NPCSlot slot, boolean auto) {
    ...
    Pair<EnumItemSlot, net.minecraft.server.v1_16_R3.ItemStack> pair =
        new Pair<>(nmsSlot, CraftItemStack.asNMSCopy(item));
    PacketPlayOutEntityEquipment packet =
        new PacketPlayOutEntityEquipment(entityId, Collections.singletonList(pair));
    playerConnection.sendPacket(packet);
  }
  ...
  public void updateSkin(Skin skin) {
    ...
    this.packetPlayOutPlayerInfoAdd =
        new PacketPlayOutPlayerInfoWrapper()
            .create(PacketPlayOutPlayerInfo.EnumPlayerInfoAction.ADD_PLAYER, newProfile, name);
    ...
    for (Player player : Bukkit.getOnlinePlayers()) {
      ...
      connection.sendPacket(
          new PacketPlayOutEntity.PacketPlayOutEntityLook(
              ...
              false));
      connection.sendPacket(
          new PacketPlayOutEntityHeadRotationWrapper().create(npcLocation, entityId));
    }
  }
}
