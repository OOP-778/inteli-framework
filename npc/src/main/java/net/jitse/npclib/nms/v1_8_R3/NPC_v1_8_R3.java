...
public class NPC_v1_8_R3 extends NPCBase {
  ...
  public NPC_v1_8_R3(InteliNPCModule instance, List<String> lines) {
  ...
  public Hologram getPlayerHologram(Player player) {
    ...
    if (holo == null) {
      holo =
          new Hologram(
              MinecraftVersion.V1_8_R3, location.clone().add(0, 0.5, 0), getPlayerLines(player));
    }
  ...
  public void sendEquipmentPacket(Player player, NPCSlot slot, boolean auto) {
    ...
    PacketPlayOutEntityEquipment packet =
        new PacketPlayOutEntityEquipment(entityId, slot.getSlot(), CraftItemStack.asNMSCopy(item));
    playerConnection.sendPacket(packet);
  }
  ...
  public void sendAnimationPacket(Player player, NPCAnimation animation) {
    if (animation == NPCAnimation.SWING_OFFHAND) {
      throw new IllegalArgumentException(
          "Offhand Swing Animations are only available on 1.9 and up.");
    }
