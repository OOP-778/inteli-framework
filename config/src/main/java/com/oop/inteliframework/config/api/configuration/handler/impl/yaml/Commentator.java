...
public class Commentator {
  ...
  public static Map<String, List<String>> comments(String[] array) {
    array = Arrays.stream(array).filter(line -> line.trim().length() != 0).toArray(String[]::new);

    Map<String, List<String>> comments = new LinkedHashMap<>();
    InteliTriPair<String[], Integer, Integer> headerScrap =
        scrapLinesFromTill(
            array,
  ...
  private static List<String> transformDescription(String[] desc) {
    return Arrays.stream(desc).map(d -> d.trim().substring(1).trim()).collect(Collectors.toList());
  }
  ...
  private static InteliTriPair<String[], Integer, Integer> scrapLinesFromTill(
      String[] array, Predicate<String> startPredicate, Predicate<String> endPredicate) {
  ...
  private static InteliPair<String[], Integer> scrapLinesTill(
      int start, String[] array, Predicate<String> predicate) {
    List<String> linesScrapped = new ArrayList<>();
    int count = 0;
